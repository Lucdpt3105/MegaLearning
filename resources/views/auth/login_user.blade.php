...
<div class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-6 shadow-2xl">
  <div class="flex items-center gap-3 mb-5">
    <div class="size-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-fuchsia-400 grid place-items-center shadow-lg">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
        <path d="M5 12l4 4L19 6" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </div>
    <div>
      <h1 class="text-xl font-semibold">Chào mừng trở lại 👋</h1>
      <p class="text-sm text-slate-400">Đăng nhập để tiếp tục học trên MegaLearning.</p>
    </div>
  </div>
  ...
  <form method="POST" action="{{ route('login.post') }}" x-data="{show:false, caps:false, loading:false}"
    ...
    <button :disabled="loading"
            class="w-full py-2 rounded-xl bg-gradient-to-tr from-indigo-600 to-fuchsia-400 font-semibold shadow-lg hover:brightness-110 active:translate-y-px disabled:opacity-60 disabled:cursor-not-allowed">
