...
<div class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-8 shadow-2xl">
  <div class="flex items-center gap-4 mb-6">
    ...
    <div>
      <h1 class="text-2xl md:text-3xl font-semibold">Khu vực Quản trị</h1>
      <p class="text-sm md:text-base text-slate-400">Chỉ dành cho quản trị viên hệ thống.</p>
    </div>
  </div>
  ...
  <form method="POST" action="{{ route('admin.login.post') }}"
    ...
    <div>
      <label for="email" class="text-base text-slate-300">Email quản trị</label>
      ...
      <div class="relative mt-2">
        <input :type="show ? 'text' : 'password'" id="password" name="password" required
               ...
               class="w-full rounded-2xl bg-slate-900/60 border border-white/10 text-slate-100
                      focus:border-cyan-400 focus:ring-0 pr-20 px-4 py-3 text-base">
