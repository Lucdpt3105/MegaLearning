...
<div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-8">
	<h1 class="mb-6 text-center text-3xl font-extrabold tracking-tight text-gray-900">CREATE POST</h1>
	...
	<form method="POST" action="{{ route('forum.store') }}" class="space-y-6">
		...
		<div class="flex items-center justify-end gap-3">
			<a href="{{ route('forum.index', ['sort' => 'my_post']) }}"  class="inline-flex items-center rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500">cancel</a>
			<button type="submit" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">Post</button>
		</div>
	</form>
</div>
@endsection